...
class Model(nn.Module):
    ...
    def forward(self, x, idx_scale, traintype='normal'):
        self.idx_scale = idx_scale
        
        if hasattr(self.model, 'set_scale'):
            self.model.set_scale(idx_scale)

        if self.training and traintype == 'normal':
            if self.n_GPUs > 1:
                return P.data_parallel(self.model, x, range(self.n_GPUs))
            else:
                return self.model(x)
        elif self.training and traintype != 'normal' :
            return self.forward_chop(x)
        else:
            forward_function = self.forward_chop
            if self.self_ensemble:
                return self.forward_x8(x, forward_function=forward_function)
            elif self.args.data_train[0] == 'attention':
                print('test attention')
                return self.model(x)
            else:
                return forward_function(x)

    def save(self, apath, epoch, is_best=False,train_is_best=False,name=""):
        save_dirs = [os.path.join(apath, 'model_latest.pt')]

        if is_best:
            save_dirs.append(os.path.join(apath, 'model_best'+name+'.pt'))
        if train_is_best:
            save_dirs.append(os.path.join(apath, 'model_trainbest'+name+'.pt'))
        if self.save_models:
            save_dirs.append(
