...
class SRData(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''for i in range(len(pair)):
                print("pair",i)
                print(pair[i][0].shape)
                print(pair[i][1].shape)'''
        if self.train:
            pair_t = []
            for i in range(len(pair)):
                pair1 = common.set_channel(*pair[i], n_channels=self.args.n_colors)
                pair_t.append(common.np2Tensor(*pair1, rgb_range=self.args.rgb_range))
            '''for i in range(len(pair_t)):
                print(i)
                print(pair_t[i][0].shape)
                print(pair_t[i][1].shape)'''
    ...
    def _load_file(self, idx):
        if self.train:
            ...
            for s in range(len(self.scale)):
                f_lr = self.images_lr[s][idx]
                f_lr_list.append(f_lr)
            filename, _ = os.path.splitext(os.path.basename(f_hr))
            if self.args.ext == 'img' or self.benchmark:
                hr = imageio.imread(f_hr)
                for i in range(len(f_lr_list)):
                    lrlist.append(imageio.imread(f_lr_list[i]))
            elif self.args.ext.find('sep') >= 0:
                #print("enter this elif")
                with open(f_hr, 'rb') as _f:
                    hr = pickle.load(_f)
                for i in range(len(f_lr_list)):
                    with open(f_lr_list[i], 'rb') as _f:
                        lrlist.append(pickle.load(_f))

            return lrlist, hr, filename
        else:
            idx = self._get_index(idx)
    ...
    def get_patch(self, lrlist, hr):
        ...
        if self.train:
            for i in range(len(self.scale)):
                lr=lrlist[i]
                scale=self.scale[i]
                lrpatch, hrpatch = common.get_patch(
                    lr, hr,
                    patch_size=self.args.patch_size*scale,
                    scale=scale,
                    multi=(len(self.scale) > 1)
                )
                if not self.args.no_augment: lrpatch, hrpatch = common.augment(lrpatch, hrpatch)
                patchlist.append((lrpatch,hrpatch))
            return patchlist
        else:
            ih, iw = lrlist.shape[:2]
